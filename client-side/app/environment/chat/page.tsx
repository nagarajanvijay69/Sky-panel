...
import { addConversation, RootState, updateUser } from "@/app/store/store";
...
const ChatSidebar = () => {
  ...
  const addChatMessage = async () => {
    ...
    dispatch(updateUser({
      total_message: user.total_message + 1
    }));

    await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URI}/chat/addChatCount`, { userId });
                    ...
                    return <div key={i} className="bg-gray-200 rounded-md cursor-pointer flex items-center justify-between px-3 py-3"
                      ...
                      <div className="flex gap-3 justify-center items-center">
                        ...
                        <div className="">
                          ...
                          <div className="text-gray-700 text-sm">{item?.last_message?.slice(0, 30)}{item.last_message[30] && "..."}</div>
                        </div>
                      </div>
                      {
